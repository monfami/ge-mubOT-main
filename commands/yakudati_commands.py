...
class YakudatiCommands:
    """役立ちメンバーコマンドを管理するクラス"""
    
    @staticmethod
    async def find_members(interaction, member_input):
        """メンバー名またはIDからメンションを生成する関数"""
        result = []
...
def setup_yakudati_commands(bot):
    ...
    async def yakudati_command(
        ...
        # 各メンバー名を解決
        member1_formatted = await YakudatiCommands.find_members(interaction, member1)
        member2_formatted = await YakudatiCommands.find_members(interaction, member2)
        member3_formatted = await YakudatiCommands.find_members(interaction, member3)
